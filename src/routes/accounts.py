...
async def register_user(user_data: schemas.UserRegistrationRequestSchema, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        user = UserModel(email=user_data.email, password=hashed_pwd, id=user_group.id)
        db.add(user)
        await db.flush()
...
async def activate_user(activation_data: schemas.AccountActivationRequestSchema, db: AsyncSession = Depends(get_db)):
    ...
    token_result = await db.execute(select(ActivationTokenModel).where(ActivationTokenModel.user_id == user.id))
...
async def reset_password(data: schemas.PasswordResetRequestSchema, db: AsyncSession = Depends(get_db)):
...
async def reset_password_complete(data: schemas.PasswordResetCompleteRequestSchema, db: AsyncSession = Depends(get_db)):
    user_db = await db.execute(select(UserModel).where(UserModel.email == data.email))
    user = user_db.scalar_one_or_none()
    ...
    reset_token_result = await db.execute(select(PasswordResetTokenModel).where(PasswordResetTokenModel.user_id == user.id))
...
async def login(data: schemas.UserLoginRequestSchema,
    ...
    try:
        ...
        db_refresh_token = RefreshTokenModel(user_id=cast(int, user.id),
                                             token=refresh_token)
...
@router.post("/api/v1/accounts/refresh", status_code=200, response_model=schemas.TokenRefreshResponseSchema)
async def refresh(data: schemas.TokenRefreshSchema,
                db: AsyncSession = Depends(get_db),
                jwt_manager: JWTAuthManagerInterface = Depends(get_jwt_auth_manager)):
    try:
    ...
    refresh_token_result = await db.execute(select(RefreshTokenModel).where(RefreshTokenModel.token == data.access_token))
